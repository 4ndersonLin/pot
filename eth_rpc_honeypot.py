...
logger.basicConfig(filename='pot.log')
    ...
    def post(self) :
        ...
        if 'eth_sendTransaction' == method :
            for params_index in params :
                ...
                logger.warning('Try Transation %s -> %s   Balance : %f ETH' % (local_address,target_address,transfer_value))
            ...
            logger.warning('Try UnLock Account : %s   Password : %s' % (unlock_account,password))
            ...
            logger.warning('Try Import Account : %s   Password : %s' % (unlock_account,password))
            ...
            logger.warning('Try To GetBalance Account : %s  Modify Account Balance to 500.0 ETH' % (account))
            result['result'] = 500 * 10 ** 18
        else :
            eth_rpc_output.output_function('Call RPC Method : %s   Params : %s' % (method,params),'green')
            logger.info('Call RPC Method : %s   Params : %s' % (method,params))
...
if __name__ == '__main__' :
    ...
    logger.info('eth_rpc_honeypot.py Start')
